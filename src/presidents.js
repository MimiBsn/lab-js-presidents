...
function getDemocraticPresidents(presidents) {
  const democraticPresidents = presidents.filter(
...
function countYearsInOffice(presidents) {
  const presidentLeftOffice = presidents.filter(
    (president) => president.leftOffice != null
  );

  const totalOfficeYears = presidentLeftOffice.reduce((acc, president) => {
    return acc + (president.leftOffice - president.tookOffice);
  }, 0);

  return totalOfficeYears;
}

// Iteration 4 | Sort Presidents by Birth Year - `sort()`
function sortPresidentsByBirthYear(presidents) {
  const numericalBirthOrder = presidents.sort((a, b) => {
    if (a.birthYear < b.birthYear) return -1;
    if (a.birthYear > b.birthYear) return 1;
    if (a.birthYear === b.birthYear) return 0;
  });
  return numericalBirthOrder;
}

// Bonus: Iteration 5 | Age at Inauguration - `map()`
function getAgeAtInauguration(presidents) {
  const ageAtInauguration = presidents.map((president) => {
...
function getPresidentsBornAfter(presidents, year) {
  const presidentBornAfterYear = presidents.filter(
    (president) => president.birthYear > year
  );
  return presidentBornAfterYear;
}

// Bonus: Iteration 7 | Count Republican Presidents
function countRepublicanPresidents(presidents) {
  const republicanPresidents = presidents.reduce((acc, president) => {
    if (president.party === "Republican") acc++;
    return acc;
  }, 0);
...
function sortPresidentsByName(presidents) {
  const presidentByName = presidents.sort((a, b) => {
    if (a.name < b.name) return -1;
    if (a.name > b.name) return 1;
    if (a.name === b.name) return 0;
  });
  return presidentByName;
}
